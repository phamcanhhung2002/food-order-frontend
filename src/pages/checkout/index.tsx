    ...
    <div>
      ...
      <div className="flex gap-x-10 max-lg:flex-col">
        ...
        <aside className="basis-4/12 p-6 border border-1 border-[#e0e0e0] h-full">
          <div>
            {cart.listProducts.map((item: any, index: number) => (
              <ItemFoodCheckout key={index} id={item.id} />
            ))}
          </div>
          ...
          <div className="flex flex-col gap-2 text-[#4F4F4F] mb-6">
            ...
            <p className="text-base flex justify-between font-lg text-[#333]">
              ...
              <span className="font-bold">
                {Number(cart.total * 0.75 + cart.total * 0.1).toFixed(2)}$
              </span>
            </p>
          </div>

          <Button className="flex items-center bg-primary text-white w-full h-[58px] justify-center text-lg font-bold">
